...
✗ 不要返回说明文字，只返回JSON"""
    ...
    private fun parseAnalysisResult(content: String): FoodAnalysisResult {
        AIResponseSanitizer.parseSingleFoodItem(content, gson)?.let { return it }

        val jsonString = AIResponseParsing.extractJsonFromText(content)
        ...
        numericFields.forEach { field ->
            ...
            cleanedJson = quotedRegex.replace(cleanedJson) { matchResult ->
                ...
                AIResponseSanitizer.normalizeNumericLiteral(raw)?.let { "\"$field\":$it" } ?: "\"$field\":0"
            }
                ...
                return keys.firstNotNullOfOrNull { key ->
                    obj.get(key)?.takeIf { !it.isJsonNull }?.let { element ->
                        if (element.isJsonPrimitive && element.asJsonPrimitive.isNumber) {
                            ...
                            AIResponseSanitizer.normalizeNumericLiteral(element.asString)?.toFloatOrNull()
                        }
                    }
                } ?: 0f
            }
